...
def batchnorm_backward_alt(dout, cache):
    ...
    gamma, x, mu, var, eps, z = cache
    N = x.shape[0]
    dz = dout * gamma
    dvar = np.sum(dz* (x - mu) * -0.5 * np.power(var + eps, -1.5), axis = 0)
    dmean = np.sum(dz * -1 / np.sqrt(var +eps), axis = 0) + dvar * np.mean(-2 * (x - mu), axis =0)
    dx = 1 / np.sqrt(var + eps) * dz + dvar * 2.0 / N * (x-mu) + 1.0 / N * dmean
    dgamma = np.sum(z * dout, axis = 0)
    dbeta = np.sum(dout , axis = 0)
...
def max_pool_forward_naive(x, pool_param):
    ...
    # pass
    N, C, H, W = x.shape
    stride, PH, PW = pool_param['stride'], pool_param['pool_height'], pool_param['pool_width']
    outH = 1 + (H - PH) // stride
    outW = 1 + (W - PW) // stride
